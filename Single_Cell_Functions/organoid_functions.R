...
# Input files
curFilename = "AA_organoid"
curTitle = "AA vs Control"
...
dimred$Pseudotime = seuratObj$Pseudotime
...
## Plot clusters on UMAP embedding
{
  ...
  ggsave(file= 'Organoid_Hull.pdf', width=2.5, height=2.5, units="in")
...
# Pathways
{
  ...
  for(curPathway in unique(pathwayScoresZscore$Pathway)){	
    ...
    ggplot(sm3W) + geom_tile(aes(x = x, y = y, fill = Diff)) + theme_cem + 
      scale_fill_gradientn(name = "AA vs Control difference", colors = pathwayColorsDiff, limits = c(-rangeDiff, rangeDiff)) + 
...
{	
  ...
  ggplot(pathwayScoresW, aes(x=UMAP_1, y=UMAP_2)) + geom_point(aes(fill = Pseudotime, color = Pseudotime), size=.1) + theme_cem + scale_fill_gradientn(colors = annColors$Pseudotime) + scale_color_gradientn(colors = annColors$Pseudotime) + geom_polygon(data = hulls2, aes(x = x, y = y, group=CellType), fill=NA, color="black", size=0.1, alpha = 0.3) + geom_text_repel(data = clusterMedian, aes(label = CellType, group = as.factor(CellType)), size = 1.75, box.padding = .1, force = 5, color = "Black") + labs(fill = "Pseudotime",  direction = 'y') + theme(text = element_text(size=5), legend.key.size = unit(0.0, "cm"), legend.text=element_text(size=0), legend.title =element_text(size=0), axis.text.x = element_text(size =  6), axis.text.y = element_text(size = 6), axis.title.x = element_text(size = 7), axis.title.y = element_text(size = 7))#+ geom_shadowtext(data = clusterMedian, aes(x = UMAP_1,y= UMAP_2, label = CellType, group = as.factor(CellType)), size = .87, bg.colour="black")
  ggsave(file= 'Organoid_Pseudotime_umap_rainbow.pdf', width=2.5, height=2.5, units="in")
  
  ggsave(paste0(curFilename, "_Pseudotime_hull.png"), width=12, height=9)
  
  ggplot(dimredP, aes(x=Pseudotime, fill=Type)) + geom_density(alpha=0.4) + theme_cem + scale_fill_manual(values=unlist(colorsType))
  ggsave(paste0(curFilename, "_CellType_PseudoTimeGenotype.png"), width=12, height=5)
  
  ggplot(dimredP, aes(x=Pseudotime, fill=CellType)) + geom_density(alpha=0.4) + theme_cem + facet_grid(CellType~., scales="free_y") + scale_fill_manual(values=cellColors)
  ggsave(paste0(curFilename, "_CellType_PseudoTime.png"), width=12, height=12)
  
  ggplot(dimredP, aes(x=Pseudotime, fill=CellType)) + geom_density(alpha=0.4) + theme_cem + scale_fill_manual(values=cellColors)
  ggsave(paste0(curFilename, "_CellType_PseudoTime2.png"), width=12, height=5)
  
  ggplot(dimredP, aes(x=Pseudotime, fill=Type)) + geom_density(alpha=0.4, bw=0.5) + theme_cem + facet_grid(CellType~., scales="free_y") + scale_fill_manual(values=unlist(colorsType))
  ...
  ggsave("Organoid_Pseudodensity_half_1.pdf", width = 1.6, height = 2, units="in")	
  
  
  cluster.ids <- c("Enterocyte Progenitor" , "Enterocyte", "Enteroendocrine","Tuft", "Goblet", "Paneth" )
  ...
  ggplot(dimredP_half2, aes(x=Pseudotime, fill=Type)) + geom_density(alpha=0.4, bw=0.5, size = .1) + theme_cem + 
    ...
    scale_y_continuous(breaks = scales::breaks_extended(n = 3)) + 
    theme(strip.background = element_blank(), strip.text.y = element_blank(), legend.position = "none", text = element_text(size=5), axis.text.x = element_text(size =  6), axis.text.y = element_text(size = 6), axis.title.x = element_text(size = 7), axis.title.y = element_text(size = 7))
  ggsave("Organoid_Pseudodensity_half_2.pdf", width = 1.6, height = 2, units="in")	
  ...
  celltype_sample_norm2$Type = splitGet(as.character(celltype_sample_norm2$Sample), "_", 2)
  ...
  ggsave( "Organoid_frac_of_cells.pdf", width=4, height=2.5, units="in")
  

  
  ggplot(celltype_sample_norm3, aes(x = Type, y = Proportion, fill = CellType)) +
    geom_bar(stat = "identity") +
    theme_cem +
    scale_fill_manual(values = iwanthue(42)) +
  ...
  ggplot(celltype_sample_norm3, aes(x = Type, y = Proportion, fill = CellType)) + geom_bar(stat = "identity") + theme_cem + scale_fill_manual(values = iwanthue(42)) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), panel.background = element_rect(fill = "white", colour = "Black"))
...
{
  assay = Matrix::t(seuratObj@assays$RNA@counts)

  geneList = unique(c("S100a6", "Lgr5", "Ascl2", "Ly6a", "Cd55")) # geneList3
  ...
  for(curGene in geneList)
  {
    ...
    rangeDiff = max(c(abs(sm3W$AA_vs_Control)))
    
    ggplot(sm3W) + geom_tile(aes(x = x, y = y, fill = AA_vs_Control)) + theme_cem +
      scale_fill_gradientn(name = "AA vs Control", colors = geneColorsDiff, limits = c(-rangeDiff, rangeDiff)) +
      geom_polygon(data = hulls2, aes(x = x, y = y, group = CellType), alpha = 0.3, fill=NA, color="#666666", size=0.1) + ylab('UMAP_2') + xlab('UMAP_1') +
      geom_text_repel(data = clusterMedian, aes(x = UMAP_1, y = UMAP_2, label = CellType, group = as.factor(CellType)), size = 2, box.padding = .1, force = 5, color = "Black") +
      theme(legend.position = "none", text = element_text(size=5),  axis.text.x = element_text(size =  6), axis.text.y = element_text(size = 6), axis.title.x = element_text(size = 7), axis.title.y = element_text(size = 7))
    
      ggsave(file= paste0(curFilename, "_dimred_geneExpression_umapAll_densityDifference_AAvsControl_", curGene, ".pdf"), width=2.5, height=2.5, units="in")
  }
  
  setwd(curPath)
}

cell_types <- c("Stem 1", "Stem Like Progenitor", "Secretory Progenitor", "Transit Amplifying", "Enterocyte Progenitor" , "Enterocyte", "Enteroendocrine", "Goblet", "Tuft", "Paneth" )
...
for(i in cell_types){
  ...
  dimreda <- dimredaub %>% filter(Type == 'AA')
  ...
  ggplot(plot11, aes(x, y, fill=y>=0)) + geom_col(position = 'dodge', width = .0978619) + ggtitle(cell) + theme_cem + scale_fill_manual(values = c('#1b9e77', '#d95f02'))+ theme(legend.position = "none") + ylab('Density') + xlab('Pseudotime') + geom_vline(data = quart, aes(xintercept = var2), size = .25) + geom_text(data = quart, aes(var1 , maxi, label = p_asterik, fontface="bold"), position=position_dodge2(0.75), vjust=.5, inherit.aes = FALSE)
  ggsave(paste0("AA_pseudo_density_", cell, ".svg"), width = 4, height = 2.5)	
}
...
ggsave(paste0("Organoid_pseudo_density_all_cell_type.pdf"), width = 3, height = 4, units="in")	
...
Density_diff_stem_2 <- ggplot(plot11, aes(x, y, fill=y>=0)) + geom_col(position = 'dodge', width = .0978619) + theme_cem + scale_fill_manual(values = c('#d95f02'))+ theme(legend.position = "none") + ylab('Density') + xlab('Pseudotime') + geom_vline(data = quart, aes(xintercept = var2), size = .075) + geom_text(data = quart, aes(var1 , maxi, label = p_asterik), position=position_dodge2(0.2), size = 1.5, vjust=1, inherit.aes = FALSE) + theme(text = element_text(size=1),axis.text = element_text(size = 2) ,plot.title = element_text(size = 6), axis.text.x = element_text(size = 6), axis.text.y = element_text(size = 6), axis.title.x = element_blank(), axis.title.y = element_blank(), plot.margin = unit(c(.1, 0, 0, 0), "cm"))
ggsave(paste0("AA_pseudo_density_stem_2.pdf"), width = 2.2, height = 1.1, units="in")	
...
Density_diff_all <- ggplot(plot11, aes(x, y, fill=y>=0)) + geom_col(position = 'dodge', width = .0978619) + ggtitle('All Cells') + theme_cem + scale_fill_manual(values = c( '#d95f02'))+ theme(legend.position = "none") + ylab('Density') + xlab('Pseudotime') + geom_vline(data = quart, aes(xintercept = var2), size = .075) + geom_text(data = quart, aes(var1 , maxi, label = p_asterik), position=position_dodge2(0.2), size = 1.5, vjust=1, inherit.aes = FALSE) + theme(text = element_text(size=1),axis.text = element_text(size = 2) ,plot.title = element_blank(), axis.text.x = element_blank(), axis.text.y = element_text(size = 6), axis.title.x = element_blank(), axis.title.y = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(.2, 0, 0, 0), "cm"))
...
## Trajectory analysis
{
  ...
  model_organoid = model
  model_organoid <- readRDS('organoid_pseudotime.rds', refhook = NULL)
  ...
  pheatmap(seuratExpSelControl, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_byCells_Control.png"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nControl Cells")
  pheatmap(seuratExpSelControl, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_byCells_Control.pdf"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nControl Cells")
