...
# Input files
curFilename = "Arasco_Arasco"
curTitle = "Arasco vs Control"

seuratAll = arasco.obj
...
## Plot clusters on UMAP embedding
{
	...
	ggsave(file= 'Arasco_Hull.pdf', width=2.5, height=2.5, units="in")
...
# Pathways
{
	...
	for(curPathway in unique(pathwayScoresZscore$Pathway)){	
		...
		sm3W$Diff = sm3W$Arasco - sm3W$Control
		rangeDiff = max(abs(sm3W$Diff))
		
		ggplot(sm3W) + geom_tile(aes(x = x, y = y, fill = Diff)) + theme_cem + 
		  scale_fill_gradientn(name = "Arasco vs Control difference", colors = pathwayColorsDiff, limits = c(-rangeDiff, rangeDiff)) + 
		...
		#ggsave(paste0("umap_densityDifference/", curFilename, "pathwayScoreZscore_umap_densityDifference_", curPathway, "_ArascoVsControl.png"), width = dimredPlotWidth, height = dimredPlotHeight, dpi=150)
		#ggsave(paste0("umap_densityDifference/", curFilename, "pathwayScoreZscore_umap_densityDifference_", curPathway, "_ArascoVsControl.pdf"), width = dimredPlotWidth, height = dimredPlotHeight)		
	}
	setwd(curPath)
...
{	
	...
	ggsave(file= 'Arasco_Pseudotime_umap_rainbow.pdf', width=2.5, height=2.5, units="in")
	...
	ggsave("Arasco_Pseudodensity_half_1.pdf", width = 1.6, height = 2, units="in")	
	...
	ggplot(dimredP_half2, aes(x=Pseudotime, fill=Type)) + geom_density(alpha=0.4, bw=0.5, size = .1) + theme_cem + 
	  ...
	  scale_y_continuous(breaks = scales::breaks_extended(n = 3)) + 
	  theme(strip.background = element_blank(), strip.text.y = element_blank(), legend.position = "none", text = element_text(size=5), axis.text.x = element_text(size =  6), axis.text.y = element_text(size = 6), axis.title.x = element_text(size = 7), axis.title.y = element_text(size = 7))
	ggsave("Arasco_Pseudodensity_half_2.pdf", width = 1.6, height = 2, units="in")	
...
## Plot gene expression on dimred embedding
{
  ...
  #assay = t(seuratObj@assays$RNA@counts)
	...
	for(curGene in geneList)
	{
		...
		sm3W = dcast(sm3, x+y~type, value.var="score")
		sm3W$Arasco_vs_Control = sm3W$Arasco - sm3W$Control
		rangeDiff = max(c(abs(sm3W$Arasco_vs_Control)))
		
		ggplot(sm3W) + geom_tile(aes(x = x, y = y, fill = Arasco_vs_Control)) + theme_cem +
		  scale_fill_gradientn(name = "Arasco vs Control", colors = geneColorsDiff, limits = c(-rangeDiff, rangeDiff)) +
		  geom_polygon(data = hulls2, aes(x = x, y = y, group = CellType), alpha = 0.3, fill=NA, color="#666666", size=0.1) + ylab('UMAP_2') + xlab('UMAP_1') +
		  geom_text_repel(data = clusterMedian, aes(x = UMAP_1, y = UMAP_2, label = CellType, group = as.factor(CellType)), size = 2, box.padding = .1, force = 5, color = "Black") +
		  theme(legend.position = "none", text = element_text(size=5),  axis.text.x = element_text(size =  6), axis.text.y = element_text(size = 6), axis.title.x = element_text(size = 7), axis.title.y = element_text(size = 7))
		
		ggsave(file= paste0(curFilename, "_dimred_geneExpression_umapAll_densityDifference_ARAvsControl_", curGene, ".pdf"), width=2.5, height=2.5, units="in")
	}
...
for(i in cell_types){
  ...
  dimreda <- dimredaub %>% filter(Type == 'Arasco')
  ...
  ggsave(paste0("ARAsco_pseudo_density_", cell, ".pdf"), width = 2.2, height = 1, units="in")	
  }
...
ggsave(paste0("ARAsco_pseudo_density_Stem_1.pdf"), width = 2.2, height = 1, units="in")	
...
ggsave(paste0("Arasco_pseudo_density_all_cell_type.pdf"), width = 3, height = 4, units="in")	
...

{
  ...
  ggsave(paste0("Arasco_pseudo_density.pdf"), width = 2.2, height = 3.1, units="in")	
  ...
  ggsave(paste0("Arasco_pseudo_density_all.pdf"), width = 2.2, height = 3.1, units="in")	
...
## Trajectory analysis
{
	...
	model_arasco = model
	model <- readRDS('arasco_pseudotime.rds', refhook = NULL)
	...
	#seuratExpSelArasco = seuratExpSel[, colnames(seuratExpSel) %in% dimred$Cell[ dimred$Type == "Arasco"] ]
	...
	pheatmap(seuratExpSelArasco, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn, filename = paste0(curFilename, "_heatmapByPseudotime_byCells_Arasco.png"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nArasco Cells")
	pheatmap(seuratExpSelArasco, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn,  filename = paste0(curFilename, "_heatmapByPseudotime_byCells_Arasco.pdf"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nArasco Cells")
	...
	seuratExpSelMedSumW_Case = as.matrix(setRemoveRownames(dcast(seuratExpSelMedSum2[ seuratExpSelMedSum2$Type == "Arasco", ], Gene~PseudotimeFactor, value.var="Expression")))
	...
	pheatmap(seuratExpSelMedSumW_Case, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn2, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_medianExpression_Arasco.png"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nMedian expression across pseudotime, Arasco Cells")
	pheatmap(seuratExpSelMedSumW_Case, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn2, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_medianExpression_Arasco.pdf"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nMedian expression across pseudotime, Arasco")
	...
	pheatmap(seuratExpSelMedSumW_Diff, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn3, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_medianExpressionDifference_ArascovsCtrl.png"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nArasco - Control median expression difference across pseudotime")
	pheatmap(seuratExpSelMedSumW_Diff, scale="none", cluster_rows = F, cluster_cols = F, show_colnames = F, breaks = breaks, treeheight_row = 0, treeheight_col = 0, color = colScale, annotation_col = annColumn3, annotation_colors = annColors, filename = paste0(curFilename, "_heatmapByPseudotime_medianExpressionDifference_ArascovsCtrl.pdf"), width=heatmapWidth, height=heatmapHeight, border_color = NA, main = "Heatmap of top 100 genes by pseudotime importance\nArasco - Control median expression difference across pseudotime")
