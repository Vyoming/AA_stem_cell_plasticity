#Vyom Shah 12/2023 - CREBKO Project 
...
samples <- read.csv('~/analysis/Crebko_Samplesheet.csv')
samples
...
pdf("correlation_CREB1.pdf")
plot(res.cnt3, contrast=2)
dev.off()
...
pdf("Boxplot_CREB1.pdf")
dba.plotBox(res.cnt3, contrast=2)
dev.off()

# plotting overlaps of sites bound by REST in different cell types
pdf("binding_site_overlap.pdf")
dba.plotVenn(res.cnt3, contrast=c(2, 3))
dev.off()

pdf("CREBKO_PCA.pdf")
dba.plotPCA(res.cnt3,contrast=2,label=DBA_CONDITION)
dev.off()

pdf("CREBKO_plotprofile.pdf")
dba.plotProfile(res.cnt3,contrast=2,)
dev.off()
...
res.db1 = dba.report(res.cnt3, contrast=2)
head(res.db1)
gr <- res.db1
...
write.csv(Anno_output, 'CREB1_KOvsWT_DiffPeak.csv')
Anno_output$Fold
...
AA_scrna <- read.csv('All_ARDWT_scrna_DE.csv')
AA_scatac <- read.csv('All_ARDvsWT_ATAC_DE.csv')

AA_scrna <- AA_scrna[AA_scrna$p_val_adj <.05 & AA_scrna$avg_log2FC >0,]
AA_scatac <- AA_scatac[AA_scatac$p_val_adj <.05 & AA_scatac$avg_log2FC >0,]
ARD_induced <- unique(intersect(AA_scrna$X, AA_scatac$gene))


YAP_scrna <- read.csv('All_ARDvYAPKO_scrna_DE.csv')
YAP_atac <- read.csv('All_ARDvsKO_YAP_ATAC_DE.csv')
CREB_atac <- read.csv('All_ARDvsKO_CREB_ATAC_DE.csv')
CREB_scrna <- read.csv('All_ARDvCREBKO_scrna_DE.csv')

YAP_scrna <- YAP_scrna[YAP_scrna$p_val_adj <.05 & YAP_scrna$avg_log2FC >0,]
YAP_atac <- YAP_atac[YAP_atac$p_val_adj <.05 & YAP_atac$avg_log2FC >0,]
CREB_atac <- CREB_atac[CREB_atac$p_val_adj <.05 & CREB_atac$avg_log2FC >0,]
CREB_scrna <- CREB_scrna[CREB_scrna$p_val_adj <.05 & CREB_scrna$avg_log2FC >0,]

ARD_induced_creb <- intersect(unique(intersect(CREB_scrna$X, CREB_atac$gene)), ARD_induced)
ARD_induced_yap <- intersect(unique(intersect(YAP_scrna$X, YAP_atac$gene)), ARD_induced)
...
gr1 <- addGeneIDs(annotatedPeak=gr,
                        orgAnn="org.Mm.eg.db",
                        IDs2Add="symbol")
gr2 <- gr1[gr1$symbol %in% ARD_induced_creb]
...
write.table(df, file="~/analysis/ARDKO_induced_creb.bed", quote=F, sep="\t", row.names=F, col.names=F)
...
write.table(df, file="~/analysis/random_regions.bed", quote=F, sep="\t", row.names=F, col.names=F)
