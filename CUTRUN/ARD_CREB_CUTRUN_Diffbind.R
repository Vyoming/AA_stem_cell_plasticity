...
samples <- read.csv('~/analysis/ARD_CREB_CUTRUN_Samplesheet.csv')
samples
...
res.cnt2$DESeq2
Condition_WT_creb1_vs_KO_creb1
...
pdf("CREB1_ARD_PCA.pdf")
dba.plotPCA(res.cnt3,contrast=1,label=DBA_CONDITION)
dev.off()

pdf("CREB1_ARD_plotprofile.pdf")
dba.plotProfile(res.cnt3,contrast=1)
dev.off()

#saveRDS(res.cnt3, file = "./data/CREBKO_Processing.rds")
#res.cnt3 <- readRDS('~/data/CREBKO_Processing.rds', refhook = NULL)
res.cnt3$samples
res.cnt3
...
write.csv(Anno_output, 'CREB1_ARDvsWT_DiffPeak.csv')
Anno_output$Fold
...
write.csv(gsea_output, paste0('CREBKO_CUTRUN_gsea_pathways.csv'))

pathways <- c('WP_NOTCH_SIGNALING', 'WNT_SIGNALING', 'WP_PROSTAGLANDIN_SIGNALING', 'WP_FATTY_ACID_BETAOXIDATION', 'BIOCARTA_MONOCYTE_PATHWAY', 'BIOCARTA_LYMPHOCYTE_PATHWAY', 'REACTOME_PKA_MEDIATED_PHOSPHORYLATION_OF_CREB', 'REACTOME_GLYCOGEN_METABOLISM', 'REACTOME_GPER1_SIGNALING', 'PID_PS1_PATHWAY')      
...
ggsave(file = paste0('Crebko_GSEA.pdf'), plot=All, width=4.75, height=3, units="in")
...
peaks.KO_creb1_R1 <- peaks.consensus[res.cnt3$called[,1]==1]
peaks.KO_creb1_R2 <- peaks.consensus[res.cnt3$called[,2]==1]

peaks.WT_creb1_R1 <- peaks.consensus[res.cnt3$called[,5]==1]
peaks.WT_creb1_R2 <- peaks.consensus[res.cnt3$called[,6]==1]


# adding an unified affinity scores column (re-formatting data)
peaks.KO_creb1_R1$Score <- peaks.KO_creb1_R1$KO_creb1_R1
peaks.KO_creb1_R2$Score <- peaks.KO_creb1_R2$KO_creb1_R2
peaks.WT_creb1_R1$Score <- peaks.WT_creb1_R1$WT_creb1_R1
peaks.WT_creb1_R2$Score <- peaks.WT_creb1_R2$WT_creb1_R2

# plotting coverage for replicate 1, using affinity scores as a weight for peaks height
covplot(peaks.KO_creb1_R1, weightCol = "Score")
covplot(peaks.WT_creb1_R1, weightCol = "Score")

# extracting peaks for each cell line present across replicates
peaks.KO_creb1 <- peaks.consensus[res.cnt3$called[,1]==1 & res.cnt3$called[,2]==1]
peaks.WT_creb1 <- peaks.consensus[res.cnt3$called[,5]==1 & res.cnt3$called[,6]==1]
...
newStyle <- mapSeqlevels(seqlevels(peaks.KO_creb1_R1), "UCSC")
peaks.KO_creb1 <- renameSeqlevels(peaks.KO_creb1, newStyle)
newStyle <- mapSeqlevels(seqlevels(peaks.WT_creb1), "UCSC")
peaks.WT_creb1 <- renameSeqlevels(peaks.WT_creb1, newStyle)

tagMatrix.1 <- getTagMatrix(peaks.KO_creb1, windows=promoter)
tagMatrix.2 <- getTagMatrix(peaks.WT_creb1, windows=promoter)
...
write.table(df, file="~/analysis/All_mouse.bed", quote=F, sep="\t", row.names=F, col.names=F)
...
AA1 <- readPeakFile(samples$Peaks[1])
AA2 <- readPeakFile(samples$Peaks[2])
AA3 <- readPeakFile(samples$Peaks[4])
C1 <- readPeakFile(samples$Peaks[3])
C2 <- readPeakFile(samples$Peaks[5])
C3 <- readPeakFile(samples$Peaks[6])


newStyle <- mapSeqlevels(seqlevels(peak), "UCSC")
AA_r1 <- renameSeqlevels(AA1, newStyle)
AA_r2 <- renameSeqlevels(AA2, newStyle)
AA_r3 <- renameSeqlevels(AA3, newStyle)
C_r1 <- renameSeqlevels(C1, newStyle)
C_r2 <- renameSeqlevels(C2, newStyle)
C_r3 <- renameSeqlevels(C3, newStyle)

promoter <- getPromoters(TxDb=txdb, upstream=2000, downstream=2000)
promoter <- annotatePeakInBatch(promoter, AnnotationData=TSS.mouse.GRCm38)
promoter <- addGeneIDs(annotatedPeak=promoter,
                  orgAnn="org.Mm.eg.db",
                  IDs2Add="symbol")
promoter <- promoter[promoter$symbol %in% AA_induced_invivo]

tagMatrixList <- lapply(GRangesList(AA_r1 = AA_r1, AA_r2 = AA_r2, AA_r3= AA_r3, C_r1 = C_r1, C_r2 = C_r2,C_r3 =  C_r3), getTagMatrix, windows=promoter)
tagMatrixList_AA <- lapply(GRangesList(AA_r1 = AA_r1, AA_r2 = AA_r2, AA_r3= AA_r3, C_r1 = C_r1, C_r2 = C_r2,C_r3 =  C_r3), getTagMatrix, windows=promoter)

plotAvgProf(tagMatrixList, xlim=c(-2000, 2000))
plotAvgProf(tagMatrixList_AA, xlim=c(-2000, 2000))

tagHeatmap(tagMatrixList_AA)


peakAnnoList <- lapply(GRangesList(AA_r1 = AA_r1, AA_r2 = AA_r2, AA_r3= AA_r3, C_r1 = C_r1, C_r2 = C_r2, C_r3 =  C_r3), annotatePeak, TxDb=txdb,
                       tssRegion=c(-3000, 3000), verbose=FALSE)
plotAnnoBar(peakAnnoList)
plotDistToTSS(peakAnnoList)
as.data.frame(peakAnnoList$AA_r1)
...
for (i in names(genes)) {
  genes[[i]] <-  getSYMBOL(genes[[i]], data='org.Mm.eg.db') %>% as.vector() 
}
AA_peaklist <- c(genes$AA_r1, genes$AA_r2, genes$AA_r3)
C_peaklist <- c(genes$C_r1, genes$C_r2, genes$C_r3)


AA_peaklist <- table(AA_peaklist)
AA_peaklist <- as.data.frame(AA_peaklist)
colnames(AA_peaklist) <- c("Gene", "Ranks")

C_peaklist <- table(C_peaklist)
C_peaklist <- as.data.frame(C_peaklist)
colnames(C_peaklist) <- c("Gene", "Ranks")

#Enrichment plots
AA_ranks <- deframe(AA_peaklist[rev(order(AA_peaklist$Ranks)),])
C_ranks <- deframe(C_peaklist[rev(order(C_peaklist$Ranks)),])
...
library(limma)
All <- plotEnrichment(AA_induced_invivo, AA_ranks) + labs(title="AA Induced Genes | NES: 0.870 | Padj: 0.954") + theme_vyom +
...
All
ggsave(file = 'AA_induced_Genes_peak_enrichment_AA_CREB.pdf', plot=All, width=3, height=3, units="in")

All <- plotEnrichment(AA_induced_invivo, C_ranks) + labs(title="AA Induced Genes | NES: 0.857 | Padj: 0.965") + theme_vyom +
  theme(plot.title = element_text(size=6),axis.title.x = element_text(size = 6), axis.title.y = element_text(size = 6), axis.text.x = element_text(size=6, angle = 45, hjust = 1, vjust= 1), axis.text.y = element_text(size=6))
All
ggsave(file = 'AA_induced_Genes_peak_enrichment_WT_CREB.pdf', plot=All, width=3, height=3, units="in")

All <- plotEnrichment(AA_induced_invivo, C_ranks) + plotEnrichment(AA_induced_invivo, AA_ranks) + xlim(0,2500) + labs(title="AA Induced Genes | NES: 0.857 | Padj: 0.965") + theme_vyom +
  theme(plot.title = element_text(size=6),axis.title.x = element_text(size = 6), axis.title.y = element_text(size = 6), axis.text.x = element_text(size=6, angle = 45, hjust = 1, vjust= 1), axis.text.y = element_text(size=6))
All

pathways.hallmark <- list(AA_induced_invivo)
names(pathways.hallmark) <- c('AA_induced_invivo')
...
fgseaRes_AA <- fgsea(pathways=pathways.hallmark, stats=AA_ranks)

fgseaResTidy_AA <- fgseaRes_AA %>%
  as_tibble() %>%
  arrange(desc(NES))

fgseaRes_C <- fgsea(pathways=pathways.hallmark, stats=C_ranks)
...
gsea_output <- as.data.frame(fgseaResTidier)
gsea_output$leadingEdge <- NULL
write.csv(gsea_output,'ENDO_mouse_GSEA.csv')
